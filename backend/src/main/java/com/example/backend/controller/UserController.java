...
public class UserController {
    ...
    // Update profile image for a user (multipart)
    @PostMapping(value = "/user/{username}/profileImage", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<?> updateProfileImage(
        ...
        try {
            ...
            return ResponseEntity.ok(java.util.Map.of("message", "Profile image updated", "fileName", fileName));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(java.util.Map.of("message", "Failed to update profile image", "error", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<byte[]> getProfileImage(@PathVariable String fileName) {
        ...
        // Pick content type by file extension; default to octet-stream
        MediaType contentType = MediaType.APPLICATION_OCTET_STREAM;
        String lower = fileName.toLowerCase();
        if (lower.endsWith(".jpg") || lower.endsWith(".jpeg")) contentType = MediaType.IMAGE_JPEG;
        else if (lower.endsWith(".png")) contentType = MediaType.IMAGE_PNG;
        else if (lower.endsWith(".gif")) contentType = MediaType.IMAGE_GIF;

        return ResponseEntity.ok().contentType(contentType).body(imageContent);
    }

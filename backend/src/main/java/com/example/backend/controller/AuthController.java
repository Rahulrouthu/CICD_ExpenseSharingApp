...
public ResponseEntity<?> registerUser(
    ...
    if (profileImage != null) {
        log.info("Received profile image: {} ({} bytes)", profileImage.getOriginalFilename(), profileImage.getSize());
    } else {
        log.warn("No profile image received in request");
    }
    ...
    try {
        ...
        if (profileImage != null && !profileImage.isEmpty()) {
            // Basic size guard (server also has multipart limits)
            if (profileImage.getSize() > 5 * 1024 * 1024) {
                return ResponseEntity.status(HttpStatus.PAYLOAD_TOO_LARGE)
                        .body(Map.of("message", "Profile image is too large. Max 5MB."));
            }
            String fileName = fileUploadUtil.uploadFileAndSaveToDatabase(profileImage);
        ...
        log.error("Data integrity violation during registration: {}", dive.getMessage(), dive);
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(Map.of(
                        "message", "Duplicate data",
                        "error", dive.getMostSpecificCause() != null ? dive.getMostSpecificCause().getMessage() : dive.getMessage()
                ));
    } catch (Exception e) {
        log.error("Registration failed for username {}: {}", user.getUsername(), e.getMessage(), e);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(Map.of(
